...
pub fn get_shipment_count(env: &Env) -> u32 {
    env.storage()
        .instance()
        .get(&soroban_sdk::symbol_short!("SCOUNT"))
        .unwrap_or(0u32)
}
...
pub fn set_shipment_count(env: &Env, n: u32) {
    env.storage()
        .instance()
        .set(&soroban_sdk::symbol_short!("SCOUNT"), &n);
}
...
pub fn set_threshold(env: &Env, t: u32) {
    env.storage()
        .instance()
        .set(&soroban_sdk::symbol_short!("THRESH"), &t);
}
...
pub fn set_circuit_breaker(env: &Env, v: bool) {
    env.storage()
        .instance()
        .set(&soroban_sdk::symbol_short!("CB"), &v);
}
...
pub fn set_shipment(env: &Env, s: &Shipment) {
    env.storage()
        .persistent()
        .set(&(soroban_sdk::symbol_short!("SHIP"), s.id), s);
}
...
pub fn set_prov_count(env: &Env, shipment_id: u32, n: u32) {
    env.storage()
        .persistent()
        .set(&(soroban_sdk::symbol_short!("PCNT"), shipment_id), &n);
}
...
pub fn get_source(env: &Env, addr: &Address) -> Option<DataSource> {
    env.storage()
        .persistent()
        .get(&(soroban_sdk::symbol_short!("SRC"), addr.clone()))
}

pub fn set_source(env: &Env, ds: &DataSource) {
    env.storage()
        .persistent()
        .set(&(soroban_sdk::symbol_short!("SRC"), ds.address.clone()), ds);
}

pub fn source_exists(env: &Env, addr: &Address) -> bool {
    env.storage()
        .persistent()
        .has(&(soroban_sdk::symbol_short!("SRC"), addr.clone()))
}
