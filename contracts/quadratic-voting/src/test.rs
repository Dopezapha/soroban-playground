...
fn test_create_proposal_ok() {
    ...
    client.initialize(&admin, &None, &None).unwrap();
    let id = client
        .create_proposal(&admin, &title(&env), &desc(&env), &None)
        .unwrap();
    assert_eq!(id, 0);
...
fn test_create_proposal_empty_title_fails() {
    ...
    client.initialize(&admin, &None, &None).unwrap();
    let err = client
        .create_proposal(&admin, &String::from_str(&env, ""), &desc(&env), &None)
        .unwrap_err();
...
fn test_non_admin_cannot_create_proposal() {
    ...
    let other = Address::generate(&env);
    let err = client
        .create_proposal(&other, &title(&env), &desc(&env), &None)
        .unwrap_err();
    assert_eq!(err, Error::Unauthorized);
}
...
fn test_finalize_quorum_not_reached() {
    ...
    // quorum = 400 bps = 4%
    client
        .initialize(&admin, &Some(1u64), &Some(400i128))
        .unwrap();
    let voter = Address::generate(&env);
