...
pub fn set_offer(env: &Env, provider: Address, offer: &StorageOffer) {
    env.storage()
        .persistent()
        .set(&DataKey::Offer(provider), offer);
}

pub fn get_total_storage(env: &Env) -> u64 {
    env.storage()
        .instance()
        .get(&DataKey::TotalStorage)
        .unwrap_or(0)
}
...
pub fn add_storage(env: &Env, amount: u64) {
    ...
    env.storage()
        .instance()
        .set(&DataKey::TotalStorage, &(total + amount));
}
...
pub fn set_provider(env: &Env, provider: Address, data: &StorageProvider) {
    ...
    if new_provider {
        ...
        env.storage()
            .instance()
            .set(&DataKey::ProviderCounter, &(counter + 1));
    } else {
    ...
    env.storage()
        .persistent()
        .set(&DataKey::Providers(index), data);

    // Always update the address -> index mapping for the provider stored in data
    env.storage()
        .persistent()
        .set(&DataKey::ProviderByAddress(data.provider.clone()), &index);
}
...
pub fn get_all_provider_ids(env: &Env) -> Vec<Address> {
    ...
    for i in 0..counter {
        if let Some(provider) = env
            .storage()
            .persistent()
            .get::<_, StorageProvider>(&DataKey::Providers(i))
        {
